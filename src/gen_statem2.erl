...
-record(state, {
  state_data = {undefined, undefined} :: {State :: term(), Data :: term()},
...
%%
loop_receive(#params{hibernate_after = HibernateAfterTimeout} = P, Debug, #state{selective = Selective} = S)
  when is_function(Selective) ->
  case Selective(HibernateAfterTimeout) of
    {receive_empty, true} ->
      loop_hibernate(P, Debug, S);
    Msg ->
      loop_receive_dispatch(P, Debug, S, Msg)
  end;

loop_receive(#params{hibernate_after = HibernateAfterTimeout} = P, Debug, S) ->
  receive
    Msg ->
      loop_receive_dispatch(P, Debug, S, Msg)
  after
...
loop_actions_list(
  ...
  case Action of
    ...
    {selective, Selective} ->
      loop_actions_list(
        P, Debug, S#state{selective = Selective}, Q, NextState_NewData,
        NextEventsR, Hibernate, TimeoutsR, Postpone,
        CallEnter, StateCall, Actions);
    {reply, From, Reply} ->
      loop_actions_reply(
